...
tpl <- mclapply(1:160, function(len) {
    lapply(1:length(forms), function(i) {
        parameterlist(1, 'DT', FALSE, forms[i], initpars[[i]], len,
                    optimized=c('cardinaltemps'))
    })
})

tpmlen <- mclapply(1:160, function(i) {
    unlist(cardinaltemps(plantmodel(wv, temps, tpl[[i]], c(0,0,0), 
                                    c(100, 100, 100), iterations=200,
                                    cores=4L)))
},  mc.cores = 4L)

saveRDS(tpmlen, paste0('DTtemps', variety,'.RDS'))
