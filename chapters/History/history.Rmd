...
Length of optimized thermal time accumulation varied significantly between crop, location and cultivar (Table 6). Accumulation lengths varied from 35 days for Mission almonds in Modesto to 74 days for prunes in Parlier. The optimized models consistently performed better than both the GDH30 models and the null model, by an average of `r round(mean(locVar$RMSE30cv - locVar$RMSEcv),2)` days and  `r round(mean(locVar$RMSEnullcv - locVar$RMSEcv),2)` days respectively.

```{r locVarTable}

kable(locVar[,c(1:4,8:10)], booktabs = TRUE,
