...
wf <- mutate(wf, heatscale=ave(Heat, cultivar, FUN=scale))

fall <- read_csv(file.path(data4, 'fall.csv'))

asl <- fall %>% filter(crop=='almond') 
psl <- fall %>% filter(crop=='prune')
wsl <- fall %>% filter(crop=='walnut')

asl <- asl %>% mutate(andscale=ave(anderson, cultivar, FUN=scale))
...
#Prune day of year for day of chill requ, agronomic chill requ and heat requ met
preqjd <- read.csv('../CA_Results/data/Prune_Cr38JD_Crag49JD_HrJD.csv',
                   stringsAsFactors = FALSE) 

# Chandler Walnut day of year for day of chill requ, agronomic chill requ and heat requ met
cwreqjd <- read.csv('../CA_Results/data/Chand_Cr42JDCrag53JDHrJD.csv',
                    stringsAsFactors = FALSE) 

# Franquette Walnut day of year for day of chill requ, agronomic chill requ and heat requ met
fwreqjd <- read.csv('../CA_Results/data/Franq_Cr51JDCrag60JDHrJD.csv',
                    stringsAsFactors = FALSE) 

# Payne Walnut day of year for day of chill requ, agronomic chill requ and heat requ met
pwreqjd <- read.csv('../CA_Results/data/Payne_Cr33JDCrag46JDHrJD.csv',
                    stringsAsFactors = FALSE) 

#Almond day of year for chill, ag chill, heat requs being met. Chico and Modesto, Sonora, Nonpareil & Mission
arequjd<-read.csv('../CA_Results/data/Almond_CrCragHrJD.csv', 
                  stringsAsFactors = FALSE) 
...
wb <- ggplot(data=wf) + theme_bw() + facet_wrap(~cultivar) +
    geom_point(aes(x=heatscale, y=bloom), size=1, color=walnutcol) +
...
```{r almond_harvest, warning=FALSE, echo=FALSE}
...
```{r prune_harvest, warning=FALSE, echo=FALSE}
...
```{r walnut_harvest, warning=FALSE, echo=FALSE}
...
### Almond Timing of Spring Phenology
...
### Almond Timing of Fall Phenology
...
### Prune Timing of Spring Phenology
