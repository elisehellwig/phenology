...
In these models, thresholds are used to predict when a plant reaches a particular phenological stage, like flowering (citation). We distinguish between two broad types of models based on the unit of this threshold value: heat threshold (HT) and time threshold (TT) models. 
...
The HT model accumulates thermal time until it reaches the heatsum threshold, $\lambda_j$. Then it counts the amount of time necessary to reach the heatsum threshold, $x_i(\lambda_j)$, and uses that to predict the length of stage $j$ (see Table A, Figure 1). Accounting for error $\varepsilon_i$, the stage length $y_{ij}$ is

\begin{equation} 
y_{ij} = \alpha_{\lambda j} + \beta_{\lambda j} \cdot x_i(\lambda_j) + \varepsilon_i
\end{equation}
...
```{r varDefs, out.width='75%', fig.align='center'}

include_graphics('images/varDefsLongA.png', dpi=150)

```



Based on equation (1), $\alpha_{j\lambda}$ is the stage length if the heatsum threshold is zero. While this is not biologically possible, it is necessary to consider mathematically because of the way the model parameters are estimated (see Table A). Additionally, $\beta_{j\lambda}$ is the change in predicted season length if the time necessary to meet the the heatsum threshold increases by one day. This can be recognized as the ordinary linear model. 

Next let us assume the heatsum threshold represents the total amount of thermal time, the plant in question requires to complete a particular developmental stage. We will refer to the heatsum threshold in this case as $\Lambda_j$. This assumption precipitates two simplifications in equation (1). First, $\alpha_{j\lambda}=0$. This is because if the the total amount of thermal time required to complete the stage is zero, then the stage length will also be zero, and this is the definition of $\alpha_{j\lambda}$ (see Table A).

Second, $\beta_{j\lambda}=1$. We know this based on the definition of $\beta_j$. $\Lambda_j$ is exactly the amount of thermal time necessary for a plant to complete a developmental stage. If the time it takes to reach the heatsum increases by one day, the plant development time increases by a day as well. Substituting these statements into equation (1) we get
...
kable(table1[,c(1,2,5,3,4)], col.names=c('Cultivar', 'Years with data',
      ...
      row.names=FALSE, align='c', format='latex', booktabs=TRUE)
...
kable(combos, col.names = c('Model', 'Type','Thermal Time Function'),
      caption = 'Model and thermal time function combinations investigated in this study. TT - Time Threshold, HT - Heatsum Threshold.', align='c', format='latex',
      booktabs=T)
...
kable(optmods2, digits=2, col.names=c('Cultivar','Model', 'Type', 'Function',
                                      'RMSE'), 
      caption='Optimal model by cultivar. RMSE determined by 5-fold crossvalidation.', row.names=FALSE, align='c', format='latex', booktabs=T)
...
kable(ms, digits=2, col.names = c('Cultivar','Type','Model','Function','Length',
      ...
      row.names=FALSE, align='c', format='latex', booktabs=T, longtable=T)
